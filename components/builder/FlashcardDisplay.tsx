    ...
    <div className="w-full h-40 [perspective:1000px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
        <div className={`relative w-full h-full transition-transform duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
            ...
            <div className="absolute w-full h-full bg-indigo-50 border border-indigo-200 rounded-lg p-4 flex items-center justify-center text-center [transform:rotateY(180deg)] [backface-visibility:hidden]">
                <div>
                    <div className="text-xs font-semibold text-indigo-400 mb-2 uppercase tracking-wider">Answer</div>
...
export const FlashcardDisplay: React.FC<{ cards: FlashcardType[], topic: string }> = ({ cards, topic }) => (
    <div className="bg-transparent">
        <h3 className="text-2xl font-bold mb-1 text-gray-800">Flashcards</h3>
        <p className="text-gray-500 mb-6">Topic: <span className="font-medium text-gray-700">{topic}</span></p>
